...
# ── M6: ODE steps 3/5/10 (재학습 불필요, eval만) ───────────────────────────
echo "[M6] ODE steps eval..."
for S in 3 5 10; do
    $PY scripts/eval_closedloop.py \
        ...
        --ode-steps $S \
        --out logs/cl_eval_ode${S}_regular.json \
        > logs/cl_eval_ode${S}.log 2>&1
    echo "  ODE=$S: $(grep 'Success Rate' logs/cl_eval_ode${S}.log)"
done
echo "[M6] 완료: $(date)"

# ── M5: 증강 없는 버전 ─────────────────────────────────────────────────────
echo "[M5] no-aug 학습 시작..."
$PY training/train.py --config configs/train_no_aug.yaml \
    > logs/train_no_aug.log 2>&1
echo "[M5] eval 시작..."
$PY scripts/eval_closedloop.py \
    --config configs/train_no_aug.yaml \
