#!/usr/bin/env python3
"""MoNa-pi 중간발표 PPTX v3 — 2026-05-28
Clean Light Korean Style (MIRICANVAS/Toss-inspired)
"""
...
# ── Color Palette (Clean Light Korean Style) ──────────────────────────────────
...
def card(sl, x, y, w, h, border_clr=None, border_w=1.0, fill=None, left_accent=None):
    ...
    if left_accent is not None:
        rect(sl, x, y, Inches(0.055), h, fill=left_accent)
    return s

def hdr(sl, title, subtitle=None):
    # Left accent bar (6px wide, navy)
    rect(sl, Inches(0.469), Inches(0.240), Inches(0.055), Inches(0.680), fill=C_NAVY)
    sans(sl, title,
         Inches(0.563), Inches(0.260), Inches(9.062), Inches(0.420),
         sz=20, bold=True, clr=C_TEXT, align=LEFT)
    if subtitle:
        sans(sl, subtitle,
             Inches(0.563), Inches(0.680), Inches(9.062), Inches(0.240),
             sz=11, clr=C_MUTED)
...
def chart_architecture():
    ...
    def fbox(x, y, w, h, title, sub='', ec=NAVY_PLT, alpha_fill=0.10):
    ...
    def arr(x1, y1, x2, y2, c=NAVY_PLT):
        ax.annotate('', xy=(x2, y2), xytext=(x1, y1),
                    arrowprops=dict(arrowstyle='->', color=c,
                                   lw=1.6, connectionstyle='arc3,rad=0.0'),
                    zorder=5)

    fbox(0.1, 8.1, 4.3, 1.1, '8-Frame Fish-eye Images', '(B, 8, H, W, 3)', ec=TEXT_PLT, alpha_fill=0.06)
    fbox(5.3, 8.1, 4.3, 1.1, 'Natural Language Command', 'instruction_pool (15 paraphrases/cat)', ec=TEXT_PLT, alpha_fill=0.06)
    fbox(0.1, 6.6, 2.0, 1.1, 'SigLIP SO400M', '14px patches', ec=NAVY_PLT)
    fbox(2.3, 6.6, 2.1, 1.1, 'Linear Proj', 'patch → token', ec=NAVY_PLT)
    fbox(5.3, 6.6, 4.3, 1.1, 'Gemma Tokenizer + Embed', 'BPE → dense (2048)', ec=NAVY_PLT)
    fbox(0.1, 5.0, 9.5, 1.2, 'Gemma-2B Language Model',
         'Full BF16 fine-tuning  →  (B, 64, 2048) context features', ec=NAVY_PLT, alpha_fill=0.14)
    fbox(0.1, 3.4, 6.8, 1.2, 'Action Expert (Cross-Attn Transformer)',
         '4-layer · 8-head · dim=256  |  AdaLN-Zero time conditioning', ec=ORANGE_PLT, alpha_fill=0.12)
    fbox(7.2, 3.4, 2.4, 1.2, 'Noisy x_t + t', 'x₀~N(0,I), t~Uniform(0,1)', ec=PURPLE_PLT, alpha_fill=0.12)
    fbox(0.1, 1.8, 9.5, 1.2, 'Flow Matching ODE  (Euler/Heun  n=5)',
         'L = ||v_θ − (x₁−x₀)||²   ·   x₁ = GT action chunk', ec=GREEN_PLT, alpha_fill=0.12)
...
def chart_flow_matching_concept():
    ...
    colors_main = [PURPLE_PLT, NAVY_PLT, GREEN_PLT]
    np.random.seed(42)
    ...
    for idx, (ax, title, col) in enumerate(zip(axes, titles, colors_main)):
        ...
        if idx == 0:
            ...
            ax.scatter(pts[:, 0], pts[:, 1], c=PURPLE_PLT, s=12, alpha=0.5, edgecolors='none')
            ...
            for s, t in zip(src, tgt):
                ax.annotate('', xy=t, xytext=s,
                            arrowprops=dict(arrowstyle='->', color=NAVY_PLT, lw=0.9, alpha=0.7))
            ...
            ax.text(-2.5, 2.5, 'v_θ', color=NAVY_PLT, fontsize=13, fontweight='bold', fontfamily='monospace')
        else:
            tgt_mean = np.array([1.0, 0.5])
            pts = tgt_mean + np.random.randn(120, 2) * 0.35
            ax.scatter(pts[:, 0], pts[:, 1], c=GREEN_PLT, s=12, alpha=0.6, edgecolors='none')
    ...
    fig.suptitle('Conditional Flow Matching: Gaussian Noise → Action Chunk (5-step ODE)',
                 color=MUTED_PLT, fontsize=9, y=0.02)
...
def chart_ablation():
    ...
    bars   = ax.bar(labels, fpe, color=colors, width=0.5, edgecolor='none', alpha=0.85)
...
def chart_comparison():
    ...
    s10    = [79.0,  76.7,  86.7]
    fpe_c  = [MUTED_PLT, BLUE_PLT, GREEN_PLT]
    s10_c  = [MUTED_PLT, BLUE_PLT, ORANGE_PLT]

    ax1 = axes[0]; _ax_style(ax1)
    bars = ax1.bar(models, fpe, color=fpe_c, width=0.55, edgecolor='none', alpha=0.85)
    for bar, v in zip(bars, fpe):
    ...
    bars2 = ax2.bar(models, s10, color=s10_c, width=0.55, edgecolor='none', alpha=0.85)
...
def chart_trajectories():
    ...
    cases = [
        ('c_str',  0.0, np.pi/2, lambda i: 0.65, lambda i: 0.00, 38, 0.582, NAVY_PLT,   '25%'),
    ...
    for i, (lbl, sx, sth, vfn, ofn, steps, fpe, col, sr) in enumerate(cases):
        ...
        ax.annotate('', xy=(px[-1], py[-1]), xytext=(gx[-1], gy[-1]),
                    arrowprops=dict(arrowstyle='->', color=MUTED_PLT, lw=0.9, alpha=0.7))
    ...
    ax.legend(handles=legend_els, loc='lower right',
              ...
              framealpha=0.9, edgecolor=GRID_PLT)
...
def chart_threshold_sweep():
    ...
    b1 = ax.bar(x - w/2, v2,  w, color=MUTED_PLT,  alpha=0.85, label='v2')
    b2 = ax.bar(x + w/2, v3a, w, color=ORANGE_PLT, alpha=0.85, label='v3-A ★')
    ax.axvspan(0.5, 1.5, color=NAVY_PLT, alpha=0.05, zorder=0)
    ax.axvline(x=1, color=NAVY_PLT, lw=1.2, linestyle='--', alpha=0.7, label='Primary (T=1.0)')
    ...
    ax.legend(facecolor=BG_PLT, labelcolor=TEXT_PLT, fontsize=7.5, edgecolor=GRID_PLT)
...
def chart_categories():
    ...
    bar_colors = [RED_PLT if s == 0 else GREEN_PLT if s >= 60 else BLUE_PLT for s in s10]
    x = np.arange(len(cats)); w = 0.35
    ax.bar(x - w/2, fpe, w, color=BLUE_PLT, alpha=0.75, label='FPE ↓', edgecolor='none')
    ax2 = ax.twinx()
    ax2.set_facecolor(BG_PLT)
    ...
    ax.set_ylabel('FPE', color=NAVY_PLT, fontsize=9)
    ax2.set_ylabel('Success@1.0 (%)', color=ORANGE_PLT, fontsize=9)
    ax.set_ylim(0, 1.8); ax2.set_ylim(0, 120)
    for sp in ax2.spines.values(): sp.set_color(GRID_PLT)
    handles = [mpatches.Patch(color=BLUE_PLT, label='FPE'),
               mpatches.Patch(color=ORANGE_PLT, label='Success@1.0')]
    ax.legend(handles=handles, loc='upper right',
              facecolor=BG_PLT, labelcolor=TEXT_PLT, fontsize=8.5, edgecolor=GRID_PLT)
...
def s01_title(prs):
    ...
    # Large title
    sans(sl, "MoNa-π",
         Inches(1.2), Inches(0.80), Inches(7.5), Inches(1.50),
         sz=82, bold=True, clr=C_NAVY, align=CENTER)

    # Subtitle white card
    card(sl, Inches(1.0), Inches(2.55), Inches(7.8), Inches(0.70),
         border_clr=C_BDR2, border_w=1.2, fill=C_WHITE)
    sans(sl, "Flow Matching 기반 고주파 모바일 내비게이션 VLA",
         Inches(1.1), Inches(2.66), Inches(7.6), Inches(0.50),
         sz=18, bold=True, clr=C_TEXT, align=CENTER)

    # Separator line
    rect(sl, Inches(3.5), Inches(3.40), Inches(2.8), Inches(0.025), fill=C_BORDER)

    # Author / date
    sans(sl, "중간발표  ·  2026. 05. 14",
         Inches(1.0), Inches(3.55), Inches(7.8), Inches(0.35),
         sz=14, clr=C_MUTED, align=CENTER)
    sans(sl, "인공지능전공 4학년 이민우  |  Team Monaf",
         Inches(1.0), Inches(3.95), Inches(7.8), Inches(0.35),
...
def s_flow_matching(prs):
    ...
    # 3 columns: gray/amber/navy accent
    card_colors = [
        (C_MUTED,  RGBColor(0xF3,0xF4,0xF6),  "Diffusion",       '"이산적 & 느림"',
         "Markov Chain\nSDE",
         [("수많은 스텝 필요", 3.576), ("연산 비용 과다", 3.934)],
         "고주파 제어 불가"),
        (C_AMBER,  C_AMBER_T,                  "CFM",             '"연속 벡터 필드"',
         "dx/dt = v_θ(x, t)",
         [("ODE 기반 모델링", 3.350), ("수학적 깔끔함", 3.707)],
         "적은 스텝 가능성"),
        (C_NAVY,   C_BLUE_T,                   "Flow Matching",   '"직선 경로 (채택 ✅)"',
         "L = ||v_θ - (x₁-x₀)||²",
    ...
    for col_idx, (ec, card_bg, ctitle, csub, ccode, bullets, footer) in enumerate(card_colors):
        x = xs[col_idx]
        # White card with colored top accent bar
        card(sl, x, Inches(1.1), cw, ch, border_clr=C_BORDER, border_w=1.0, fill=C_WHITE)
        rect(sl, x, Inches(1.1), cw, Inches(0.08), fill=ec)

        sans(sl, ctitle,
             x + Inches(0.20), Inches(1.26), cw - Inches(0.40), Inches(0.48),
             sz=18, bold=True, clr=ec)

        sans(sl, csub,
             x + Inches(0.20), Inches(1.84), cw - Inches(0.40), Inches(0.30),
             sz=13, bold=True, clr=C_TEXT)

        # Code block (tinted bg)
        rect(sl, x + Inches(0.20), Inches(2.28), cw - Inches(0.40), Inches(0.52), fill=card_bg)
        rect(sl, x + Inches(0.20), Inches(2.28), Inches(0.04), Inches(0.52), fill=ec)
        mono(sl, ccode,
             x + Inches(0.28), Inches(2.30), cw - Inches(0.52), Inches(0.48),
             sz=10, clr=ec)

        for txt, iy in bullets:
            sans(sl, "→ " + txt,
                 x + Inches(0.20), Inches(iy), cw - Inches(0.40), Inches(0.270),
                 sz=11, clr=C_TEXT)

        # Footer tinted card
        rect(sl, x + Inches(0.20), Inches(4.56), cw - Inches(0.40), Inches(0.50), fill=card_bg)
        rect(sl, x + Inches(0.20), Inches(4.56), Inches(0.04), Inches(0.50), fill=ec)
        sans(sl, footer,
             x + Inches(0.28), Inches(4.60), cw - Inches(0.52), Inches(0.42),
             sz=12, bold=True, clr=ec, align=CENTER)
...
def s02_background(prs):
    ...
    rect(sl, Inches(0.62), Inches(1.53), Inches(3.70), Inches(0.020), fill=C_BORDER)
    ...
    # Photo placeholders
    img_front_path = Path(__file__).parent / "robot_front.jpg"
    img_top_path = Path(__file__).parent / "robot_top.jpg"

    if img_front_path.exists():
        sl.shapes.add_picture(str(img_front_path), Inches(0.62), Inches(4.20), Inches(1.75), Inches(0.90))
    else:
    ...
    if img_top_path.exists():
        sl.shapes.add_picture(str(img_top_path), Inches(2.50), Inches(4.20), Inches(1.85), Inches(0.90))
    else:
    ...
    challenge_specs = [
        ...
        (C_NAVY,   "③ 언어 이해 일반화",
         ...
         C_BLUE_T),
        (C_PURPLE, "④ 실시간 배포 현실성",
         "GX10 검증 → Serbot2 (Jetson Orin AGX 16GB) 실배포: 4Hz 예산 내 추론 완료 필요",
         C_PURPLE_T),
    ]
    for i, (ec, title, detail, bg) in enumerate(challenge_specs):
        cy = Inches(1.10 + i * 1.06)
        card(sl, Inches(4.65), cy, Inches(5.0), Inches(0.95),
             border_clr=ec, border_w=1.2, fill=bg, left_accent=ec)
...
def s03_design_focus(prs):
    ...
    grid = [
        ("01", "연속 Flow Matching",
         "이산 Class 아닌 연속 경로\n단 5스텝 ODE 복원",
         "물리적으로 부드러운 연속 궤적", C_NAVY,   C_BLUE_T),
        ("02", "Action Chunking (h=10)",
         "미래 10스텝 동시 예측\n4Hz 재계획 / 50Hz 로컬 루프",
         "고주파 제어 & Jitter 감소", C_GREEN,  C_GREEN_T),
        ("03", "Instruction Pool",
         "카테고리당 15개 Paraphrase\n매 스텝 무작위 선택",
         "다양한 자연어에 강건한 일반화", C_ORANGE, C_AMBER_T),
        ("04", "BF16 학습 & 배포",
         "FP32 대비 메모리 절반\nGradient 안정성 확보",
         "GX10·Serbot2 (Jetson Orin AGX) 네이티브 지원", C_PURPLE, C_PURPLE_T),
    ]
    xs = [Inches(0.469), Inches(5.117)]
    ys = [Inches(1.10),  Inches(3.22)]
    cw, ch = Inches(4.414), Inches(1.90)

    for i, (num, title, body, effect, ec, bg) in enumerate(grid):
        col, row = i % 2, i // 2
        x, y = xs[col], ys[row]
        card(sl, x, y, cw, ch, border_clr=C_BORDER, border_w=1.0, fill=C_WHITE)
        rect(sl, x, y, cw, Inches(0.07), fill=ec)
        sans(sl, num,
        ...
        sans(sl, body,
             ...
             sz=11, clr=C_MUTED)
        # Effect row: tinted
        rect(sl, x + Inches(0.18), y + Inches(1.46), Inches(4.05), Inches(0.34), fill=bg)
        rect(sl, x + Inches(0.18), y + Inches(1.46), Inches(0.04), Inches(0.34), fill=ec)
        sans(sl, "→ " + effect,
             x + Inches(0.26), y + Inches(1.48), Inches(3.95), Inches(0.30),
...
def s04_approach(prs):
    ...
    for iy, num, from_t, arrow, to_t, detail, ec, bg in rows:
        card(sl, Inches(0.469), Inches(iy), Inches(9.062), Inches(1.17),
             border_clr=C_BORDER, border_w=1.0, fill=C_WHITE, left_accent=ec)
...
def s05_architecture(prs, buf_arch):
    ...
    rect(sl, Inches(6.45), Inches(1.50), Inches(3.20), Inches(0.020), fill=C_BORDER)

    components = [
        (C_NAVY,   "Backbone (PaliGemma 3B)",
...
def s_adaln(prs):
    ...
    card(sl, Inches(0.469), Inches(1.10), Inches(4.375), Inches(4.30),
         border_clr=C_RED, border_w=1.5, fill=C_WHITE)
    rect(sl, Inches(0.469), Inches(1.10), Inches(4.375), Inches(0.07), fill=C_RED)

    sans(sl, "BEFORE  단순 덧셈",
         Inches(0.60), Inches(1.22), Inches(4.10), Inches(0.38),
         sz=14, bold=True, clr=C_RED)

    rect(sl, Inches(0.60), Inches(1.70), Inches(4.10), Inches(1.40), fill=C_RED_T)
    ...
    rect(sl, Inches(0.60), Inches(4.30), Inches(4.10), Inches(0.65), fill=C_RED_T)
    rect(sl, Inches(0.60), Inches(4.30), Inches(0.04), Inches(0.65), fill=C_RED)
    sans(sl, "논문과 불일치 — DiT/π0 방식 미적용",
         Inches(0.68), Inches(4.43), Inches(4.02), Inches(0.38),
         sz=11, bold=True, clr=C_RED, align=CENTER)
    ...
    card(sl, Inches(5.156), Inches(1.10), Inches(4.375), Inches(4.30),
         border_clr=C_GREEN, border_w=1.5, fill=C_WHITE)
    rect(sl, Inches(5.156), Inches(1.10), Inches(4.375), Inches(0.07), fill=C_GREEN)

    sans(sl, "AFTER  AdaLN-Zero  (π0 정통)",
         Inches(5.28), Inches(1.22), Inches(4.10), Inches(0.38),
         sz=14, bold=True, clr=C_GREEN)

    rect(sl, Inches(5.28), Inches(1.70), Inches(4.10), Inches(1.85), fill=C_GREEN_T)
    ...
    rect(sl, Inches(5.28), Inches(4.30), Inches(4.10), Inches(0.65), fill=C_GREEN_T)
    rect(sl, Inches(5.28), Inches(4.30), Inches(0.04), Inches(0.65), fill=C_GREEN)
    mono(sl, "검증 완료  verify_mona_expert.py: Max Error < 1e-6",
         Inches(5.36), Inches(4.43), Inches(4.02), Inches(0.38),
         sz=10, bold=True, clr=C_GREEN, align=CENTER)
...
def s06_dataset(prs):
    ...
    rect(sl, Inches(0.62), Inches(3.38), Inches(3.95), Inches(0.020), fill=C_BORDER)

    sans(sl, "비동기 수집 파이프라인 (Serbot2 / Jetson Orin AGX 16GB)",
         Inches(0.62), Inches(3.48), Inches(3.90), Inches(0.28),
         sz=11, bold=True, clr=C_ORANGE)

    rect(sl, Inches(0.62), Inches(3.80), Inches(3.95), Inches(1.00), fill=C_AMBER_T)
    ...
    # Right column: hyperparams table
    ...
    # Header row
    rect(sl, Inches(4.95), Inches(1.52), Inches(4.60), Inches(0.26), fill=C_NAVY)
    mono(sl, "파라미터", Inches(5.08), Inches(1.54), Inches(2.00), Inches(0.22),
         sz=10, bold=True, clr=C_WHITE)
    mono(sl, "값", Inches(7.30), Inches(1.54), Inches(2.00), Inches(0.22),
         sz=10, bold=True, clr=C_WHITE)
    ...
    for i, (k, v, vc) in enumerate(hp):
        ry = Inches(1.78 + i * 0.24)
        row_fill = C_CARD2 if i % 2 == 0 else C_WHITE
        rect(sl, Inches(4.95), ry, Inches(4.60), Inches(0.24), fill=row_fill)
...
def s07_ablation(prs, buf_ab):
    ...
    for htxt, hx, hw in hdr_defs:
        rect(sl, hx, Inches(1.10), hw, row_h, fill=C_NAVY)
    ...
    for i, (cells, hl) in enumerate(data_rows):
        ...
        if hl == 'red':
            rect(sl, Inches(0.469), ry, Inches(4.40), Inches(0.51), fill=C_RED_T)
        else:
            fill = C_CARD2 if i % 2 == 0 else C_WHITE
            rect(sl, Inches(0.469), ry, Inches(4.40), Inches(0.51), fill=fill)
        for txt, cx, cw in zip(cells, col_xs, col_ws):
            tc = C_RED if hl == 'red' else C_TEXT
    ...
    sans(sl, "핵심 해석",
         ...
         sz=15, bold=True, clr=C_NAVY)

    interps = [
        (C_GREEN,  C_GREEN_T,  "Instruction 유효성",
         "text-off → FPE +61% 증가\n언어 명령이 궤적 생성에 필수적",  2.00),
        (C_RED,    C_RED_T,    "FP16 배포 불가 확정",
         "FPE 3.1× 증가 · CL Success 0%\n정밀도 손실 → 성능 완전 붕괴", 3.10),
        (C_ORANGE, C_AMBER_T,  "BF16 배포 확정",
         "GX10 & Serbot2 (Jetson Orin AGX) BF16 네이티브\nFP32 대비 메모리 절반",     4.20),
    ]
    for ec, bg, title, detail, iy in interps:
        rect(sl, Inches(5.28), Inches(iy), Inches(4.05), Inches(0.75), fill=bg)
        rect(sl, Inches(5.28), Inches(iy), Inches(0.05), Inches(0.75), fill=ec)
        sans(sl, title,
             Inches(5.38), Inches(iy + 0.04), Inches(3.85), Inches(0.28),
             sz=12, bold=True, clr=ec)
        sans(sl, detail,
             Inches(5.38), Inches(iy + 0.32), Inches(3.85), Inches(0.40),
             sz=10.5, clr=C_MUTED)
...
def s_metrics(prs, buf_traj, buf_sweep):
    ...
    card(sl, Inches(0.469), Inches(1.10), Inches(4.10), Inches(1.40),
         border_clr=C_NAVY, border_w=1.2, fill=C_WHITE, left_accent=C_NAVY)
    sans(sl, "FPE (Final Position Error)",
         Inches(0.62), Inches(1.18), Inches(3.80), Inches(0.35),
         sz=13, bold=True, clr=C_NAVY)
    rect(sl, Inches(0.62), Inches(1.58), Inches(3.80), Inches(0.40), fill=C_BLUE_T)
    mono(sl, "|| p_pred − p_gt ||₂",
         Inches(0.62), Inches(1.58), Inches(3.80), Inches(0.40),
         sz=12, clr=C_NAVY, align=CENTER)
    ...
    card(sl, Inches(0.469), Inches(2.58), Inches(4.10), Inches(1.40),
         border_clr=C_ORANGE, border_w=1.2, fill=C_WHITE, left_accent=C_ORANGE)
    sans(sl, "Success@T",
         Inches(0.62), Inches(2.66), Inches(3.80), Inches(0.35),
         sz=13, bold=True, clr=C_ORANGE)
    rect(sl, Inches(0.62), Inches(3.06), Inches(3.80), Inches(0.40), fill=C_AMBER_T)
    ...
    card(sl, Inches(4.80), Inches(4.74), Inches(4.85), Inches(0.80),
         border_clr=C_GREEN, border_w=1.2, fill=C_GREEN_T, left_accent=C_GREEN)
...
def s08_results(prs, buf_cmp):
    ...
    for i, (cells, hl) in enumerate(data_rows):
        ...
        if hl == 'orange':
            rect(sl, Inches(0.469), ry, Inches(5.25), Inches(0.44), fill=C_AMBER_T)
            tc = C_ORANGE; tb = True
        else:
            fill = C_CARD2 if i % 2 == 0 else C_WHITE
    ...
    card(sl, Inches(5.55), Inches(2.55), Inches(3.99), Inches(2.20),
         border_clr=C_RED, border_w=1.2, fill=C_RED_T, left_accent=C_RED)
    sans(sl, "Val Loss ≠ CL 성능",
    ...
    card(sl, Inches(5.55), Inches(4.85), Inches(3.99), Inches(0.68),
         border_clr=C_NAVY, fill=C_BLUE_T, left_accent=C_NAVY)
...
def s09_categories(prs, buf_cat):
    ...
    for i, (cat, n, fpe, s10, hl) in enumerate(cat_rows):
        ...
        if hl == 'red':
            rect(sl, Inches(0.469), ry, total_w, row_h, fill=C_RED_T)
            tc = C_RED; tb = True
        elif hl == 'green':
            rect(sl, Inches(0.469), ry, total_w, row_h, fill=C_GREEN_T)
            tc = C_GREEN; tb = True
        else:
            fill = C_CARD2 if i % 2 == 0 else C_WHITE
    ...
    issues = [
        (C_RED,    C_RED_T,    "시각적 모호성",
         "fish-eye 카메라에서 left/right\n구별이 어려운 구도 발생", 1.72),
        (C_ORANGE, C_AMBER_T,  "통계적 불확실성",
         "val n=3으로 샘플 수 부족\n평가 결과 신뢰도 한계",       2.52),
        (C_MUTED,  C_CARD2,    "Instruction 다양화 한계",
         "언어 표현 다양화만으로는\n시각 모호성 해결 불가능",      3.28),
    ]
    for ec, bg, title, detail, iy in issues:
        rect(sl, Inches(5.28), Inches(iy), Inches(4.05), Inches(0.62), fill=bg)
        rect(sl, Inches(5.28), Inches(iy), Inches(0.05), Inches(0.62), fill=ec)
        sans(sl, title,
             Inches(5.40), Inches(iy + 0.04), Inches(3.85), Inches(0.26),
             sz=12, bold=True, clr=ec)
        sans(sl, detail,
             Inches(5.40), Inches(iy + 0.30), Inches(3.85), Inches(0.30),
             sz=10, clr=C_MUTED)

    # Solution block
    rect(sl, Inches(5.28), Inches(4.08), Inches(4.05), Inches(0.72), fill=C_BLUE_T)
    rect(sl, Inches(5.28), Inches(4.08), Inches(0.05), Inches(0.72), fill=C_NAVY)
    sans(sl, "해결책 (H6)",
         Inches(5.38), Inches(4.12), Inches(3.90), Inches(0.28),
         sz=12, bold=True, clr=C_NAVY)
    sans(sl, "다양한 시작 위치 에피소드 추가 수집 → v4 재학습",
         Inches(5.38), Inches(4.40), Inches(3.90), Inches(0.32),
...
def s10_server(prs):
    sl = blank(prs); set_bg(sl)
    hdr(sl, "추론 서버 검증 — GX10 검증 → Serbot2 (Jetson Orin AGX 16GB) 실배포",
        "Warm 평균 ~243ms  <  4Hz 예산 250ms  →  Serbot2 실배포 가능 확인")
    ...
    sans(sl, "서버 스펙",
         ...
         sz=14, bold=True, clr=C_NAVY)
    rect(sl, Inches(0.62), Inches(1.54), Inches(3.65), Inches(0.020), fill=C_BORDER)

    specs = [
        ("GPU",   "GB10 Superchip",   C_NAVY),
        ("모델",   "v3-A BF16",        C_TEXT),
    ...
    for i, (k, v, vc) in enumerate(specs):
        ...
        row_fill = C_CARD2 if i % 2 == 0 else C_WHITE
        rect(sl, Inches(0.62), ry, Inches(3.65), Inches(0.46), fill=row_fill)
        sans(sl, k, Inches(0.72), ry + Inches(0.06), Inches(1.30), Inches(0.34), sz=12, clr=C_MUTED)
        mono(sl, v, Inches(2.10), ry + Inches(0.06), Inches(2.00), Inches(0.34), sz=12, bold=True, clr=vc)

    rect(sl, Inches(0.62), Inches(4.50), Inches(3.65), Inches(0.80), fill=C_GREEN_T)
    rect(sl, Inches(0.62), Inches(4.50), Inches(0.04), Inches(0.80), fill=C_GREEN)
    sans(sl, "Serbot2 (Jetson Orin AGX 16GB) 호환성",
         Inches(0.72), Inches(4.55), Inches(3.45), Inches(0.28),
         sz=11, bold=True, clr=C_GREEN)
    sans(sl, "5.97 GB → Serbot2 16GB 여유 충분\nBF16 네이티브 지원 → 실배포 준비 완료",
    ...
    for i, (cells, hl) in enumerate(lat_rows):
        ry = Inches(1.61 + i * 0.51)
        if hl == 'red':
            rect(sl, Inches(4.65), ry, Inches(3.88), Inches(0.51), fill=C_RED_T)
            tc = C_RED; tb = True
        elif hl == 'green':
            rect(sl, Inches(4.65), ry, Inches(3.88), Inches(0.51), fill=C_GREEN_T)
            tc = C_GREEN; tb = True
        else:
            fill = C_CARD2 if i % 2 == 0 else C_WHITE
    ...
    card(sl, Inches(4.65), Inches(4.27), Inches(3.88), Inches(1.23),
         border_clr=C_NAVY, border_w=1.2, fill=C_BLUE_T, left_accent=C_NAVY)
    mono_lines(sl, [
        ("[GX10 서버]  →  /predict API  →  (B,10,3) chunk",  False, C_NAVY),
        ("       ↓",                                           False, C_TEXT),
        ("[Serbot2 / Jetson Orin AGX 16GB]",                  True,  C_GREEN),
        ("  → Action Buffer → 50Hz 로컬 제어 → ROS2",         False, C_MUTED),
...
def s11_discussion(prs):
    ...
    rows = [
        (C_RED,    C_RED_T,    "❌", "center_left  0%  실패",
         "시각적 모호성 및 통계적 불확실성 (val n=3)",
         "→ 다양한 시작 위치 에피소드 추가 수집 (H6 milestone)",
         1.10),
        (C_ORANGE, C_AMBER_T,  "⚠", "Val Loss ≠ CL 성능 불일치",
         "단순 Loss보다 Downstream 일반화가 실질적 성능 지표",
         "→ 평가 기준: Closed-Loop Success 확정 사용",
         2.60),
        (C_NAVY,   C_BLUE_T,   "⏳", "실로봇 미배포 상태",
         "현재 시뮬레이션 폐루프(CL)만 검증 완료된 상태",
         "→ GX10 구동 완료 / Serbot2 (Jetson Orin AGX 16GB) 코드 준비 완료 / 배포 대기",
         4.10),
    ]
    for ec, bg, icon, title, muted, action, iy in rows:
...
def s12_conclusion(prs):
    ...
    card(sl, Inches(0.469), Inches(1.00), Inches(4.45), Inches(3.10),
         border_clr=C_GREEN, border_w=1.2, fill=C_WHITE)
    rect(sl, Inches(0.469), Inches(1.00), Inches(4.45), Inches(0.07), fill=C_GREEN)
    sans(sl, "주요 기여 (완료)",
    ...
    contribs = [
        ...
        ("✅", "배포 검증 완료",
         "GX10 243ms ✅  5.97GB ✅  Serbot2 (Jetson Orin AGX 16GB) 호환",  2.60),
        ("✅", "실용적 진단",
    ...
    card(sl, Inches(5.10), Inches(1.00), Inches(4.45), Inches(3.10),
         border_clr=C_ORANGE, border_w=1.2, fill=C_WHITE)
    rect(sl, Inches(5.10), Inches(1.00), Inches(4.45), Inches(0.07), fill=C_ORANGE)
    sans(sl, "향후 계획",
    ...
    plans = [
        ("📌", "단기 (D10-D11)",
         "Serbot2 실배포 · 실환경 주행 10+ ep · CL Success 측정", 1.60),
        ("📌", "중기 (H6)",
...
if __name__ == "__main__":
    ...
    s05_architecture(prs, buf_arch)         # Slide 06 — 아키텍처
    s_adaln(prs)                            # Slide 07 — AdaLN-Zero
    s06_dataset(prs)                        # Slide 08 — 데이터셋
    ...
    out = "/home/minum/26CS/MoNa-pi/reports/mona_pi_v3.pptx"
    prs.save(out)
